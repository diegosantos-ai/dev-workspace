...
# Em algumas instalacoes novas, o binario `code` ou o snap pode falhar com exit 139.
# Por padrao, o relatorio nao deve quebrar por causa do VS Code.
# Para reativar: UPDATE_TOOLS_CHECK_VSCODE=1 make update-report
UPDATE_TOOLS_CHECK_VSCODE="${UPDATE_TOOLS_CHECK_VSCODE:-0}"
...
vscode_checks_enabled() {
  [ "${UPDATE_TOOLS_CHECK_VSCODE:-0}" = "1" ]
}
...
snap_list_safe() {
  timeout 15s snap list "$@" 2>/dev/null || true
}
...
collect_code_status() {
  ...
  local code_version installed_rev pending_rev installed_pkg candidate_pkg code_rc
  ...
  code_version="$(code_version_safe)"

  if command_exists snap && snap_list_safe code | awk 'NR == 2 { found = 1 } END { exit(found ? 0 : 1) }'; then
    channel="snap:code"
    installed_rev="$(snap_list_safe code | awk 'NR == 2 { print $3 }')"
    pending_rev="$(snap_refresh_list_safe | awk '$1 == "code" { print $3; exit }')"
  ...
  if command_exists dpkg-query && dpkg_query_safe -W -f='${Status}' code | grep -q 'install ok installed'; then
    channel="apt:code"
    installed_pkg="$(dpkg_query_safe -W -f='${Version}' code)"
    candidate_pkg="$(apt_cache_policy_safe code | awk '/Candidate:/ { print $2; exit }')"
  ...
  record_status "VS Code" "${channel}" "${code_version:-desconhecida}" "" "sem-verificacao"
}
...
main() {
  ...
  if vscode_checks_enabled && command_exists code; then
    if command_exists snap && snap_list_safe code | awk 'NR == 2 { found = 1 } END { exit(found ? 0 : 1) }'; then
      code_channel="snap:code"
      if snap_refresh_list_safe | awk '$1 == "code" { found = 1 } END { exit(found ? 0 : 1) }'; then
        code_update_status="update-disponivel"
      fi
    elif command_exists dpkg-query && dpkg_query_safe -W -f='${Status}' code | grep -q 'install ok installed'; then
      code_channel="apt:code"
      if [ "$(apt_cache_policy_safe code | awk '/Installed:/ { print $2; exit }')" != \
           "$(apt_cache_policy_safe code | awk '/Candidate:/ { print $2; exit }')" ]; then
        code_update_status="update-disponivel"
      fi
    fi
  fi
