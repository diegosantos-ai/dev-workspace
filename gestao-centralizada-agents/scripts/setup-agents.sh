...
# 0. Resolucao de caminhos
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" &> /dev/null && pwd)"
REPO_ROOT="$(dirname "$(dirname "$SCRIPT_DIR")")"

# 1. Checa pipx
if ! command -v pipx &> /dev/null; then
    echo -e "${RED}[ ERRO ] pipx não encontrado. Rode 'make setup-workstation' primeiro para o Ansible instalar as dependências do SO.${RESET}"
    exit 1
fi
...
if pipx list | grep -q "crewai"; then
    echo -e "   [ INFO ] CrewAI já instalado. Rodando upgrade..."
    pipx upgrade crewai > /dev/null 2>&1 || true
else
    pipx install crewai > /dev/null 2>&1 || echo -e "   [ ERRO ] Falha ao instalar CrewAI."
fi
...
# [ MCP Skills Path ]
# Este path eh usado pelos agentes para localizar ferramentas personalizadas.
