...
## Objetivo
Você preparou sua máquina (rodou o `make setup` no `dev-workspace`). Como inicializar um novo projeto do zero agora ou adequar um projeto que já estava criado para usar todas as barreiras de segurança e automação local da sua máquina?
...
Abra o terminal **dentro do seu projeto alvo** (ex: `~/labs/meu-novo-projeto/`) e rode:

```bash
cd ~/labs/meu-novo-projeto
bash ~/docs/dev-workspace/gestao-centralizada-agents/scripts/adopt-governance.sh
```

Apenas chamando o script direto por esse atalho universal, a automação fará rigorosamente o seguinte:
